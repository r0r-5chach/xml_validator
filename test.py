...
class TestFindMainSchema:
    ...
    def test_find_fsa029_different_name(self, tmp_path):
        ...
        (tmp_path / "CommonTypes-Schema.xsd").write_text('<schema/>')
        (tmp_path / "FSA029-BalanceSheet.xsd").write_text('<schema/>')  # Contains FSA029
...
class TestValidateXml:
    ...
    def test_validate_xml_success_valid(self, mock_print, mock_file, mock_parse, 
                                       mock_schema_class, mock_find, mock_create, mock_validate):
        ...
        # Verify
        mock_print.assert_called_with("Submitted file (/submission.xml) is VALID")
    ...
    @patch('builtins.open', new_callable=mock_open)
    @patch('builtins.print')
    def test_validate_xml_success_invalid(self, mock_print, mock_file, mock_parse,
                                         mock_schema_class, mock_find, mock_create, mock_validate):
        ...
        # Verify
        mock_print.assert_called_with("Submitted file (/submission.xml) is INVALID")
    ...
    def test_validate_xml_invalid_inputs(self, mock_print, mock_validate):
        ...
        mock_print.assert_called_with("Paths provided do not lead to valid file types (xsd, xml)")
    
    @patch('xml_validator.validate_inputs')
    @patch('xml_validator.create_temp_schemas')
    @patch('xml_validator.find_main_schema')
    @patch('builtins.print')
    def test_validate_xml_no_main_schema(self, mock_print, mock_find, mock_create, mock_validate):
        ...
        mock_print.assert_called_with("Error: FSA029 schema not in the provided directory")
    
    @patch('xml_validator.validate_inputs')
    @patch('xml_validator.create_temp_schemas')
    @patch('builtins.print')
    def test_validate_xml_exception_handling(self, mock_print, mock_create, mock_validate):
    ...
    def test_validate_xml_cleanup(self, mock_rmtree, mock_find, mock_create, mock_validate):
    ...
    @patch('xml_validator.find_main_schema')
    @patch('xml_validator.shutil.rmtree')
    def test_validate_xml_cleanup_on_exception(self, mock_rmtree, mock_find, mock_create, mock_validate):
...
class TestMain:
    ...
    def test_main_no_args(self, mock_print):
        ...
        mock_print.assert_called_with("Usage: python xml_validator.py <schema_dir> <submission_file>")
    ...
    @patch('sys.argv', ['xml_validator.py', '/schemas', '/submission.xml', 'extra'])
    @patch('builtins.print')
...
<TestElement xmlns="urn:fsa-gov-uk:MER:FSA029:4">12345</TestElement>''')
    ...
    def test_integration_validate_inputs_with_fixtures(self, temp_schema_dir, temp_xml_file):
        """Test validate_inputs with fixture-created files."""
        result = validate_inputs(temp_schema_dir, temp_xml_file)
        assert result is True
    
    def test_integration_fix_schema_imports_complex(self, sample_schema_content):
    ...
    def test_integration_end_to_end(self, tmp_path):
        ...
        (schema_dir / "FSA029-Schema.xsd").write_text('''<?xml version="1.0" encoding="UTF-8"?>
<xs:schema xmlns:xs="http://www.w3.org/2001/XMLSchema">
    ...
    <xs:element name="FSA029-BalanceSheet">
        ...
        (schema_dir / "CommonTypes-Schema.xsd").write_text('''<?xml version="1.0" encoding="UTF-8"?>
<xs:schema xmlns:xs="http://www.w3.org/2001/XMLSchema">
...
<FSA029-BalanceSheet>
    ...
    def test_integration_with_import_fixing(self, tmp_path):
        ...
        schema_with_bad_imports.write_text('''<?xml version="1.0" encoding="UTF-8"?>
<xs:schema xmlns:xs="http://www.w3.org/2001/XMLSchema">
    <xs:include schemaLocation="../../CommonTypes/v14/CommonTypes-Schema.xsd"/>
    ...
    <xs:element name="TestElement"/>
        ...
        (schema_dir / "CommonTypes-Schema.xsd").write_text('<xs:schema xmlns:xs="http://www.w3.org/2001/XMLSchema"/>')
        ...
        try:
            ...
            with patch('xml_validator.etree.XMLSchema') as mock_schema_class:
                ...
                with patch('builtins.print') as mock_print:
                    ...
                    mock_print.assert_called_with(f"Submitted file ({xml_file}) is VALID")
